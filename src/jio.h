...
//
#if defined(PORTE)
   #define AVR_ATmega2560_FAMILY
#elif defined(PORTD)
   #define AVR_ATmega328_FAMILY
#endif
...
//
#if defined(AVR_ATmega328_FAMILY)
	#define _PIN_OUTPUT(pin) (IO_REG8(pgm_read_byte(ddr_lu + pin)) |= pgm_read_byte(bit_lu + pin))
	#define _PIN_INPUT(pin) (IO_REG8(pgm_read_byte(ddr_lu + pin)) &= ~(pgm_read_byte(bit_lu + pin)))
#elif defined(AVR_ATmega2560_FAMILY)
...
#if defined(AVR_ATmega328_FAMILY)
	#define _PIN_READ(pin) (bool)(IO_REG8(pgm_read_byte(pin_lu + pin)) & pgm_read_byte(bit_lu + pin))
	#define _PIN_WRITE(pin, V) do{ if(V){ IO_REG8(pgm_read_byte(port_lu + pin)) |= pgm_read_byte(bit_lu + pin); } \
		else { IO_REG8(pgm_read_byte(port_lu + pin)) &= ~(pgm_read_byte(bit_lu + pin)); } }while(0)
	#define _PORT_READ(pin) IO_REG8(pgm_read_byte(pin_lu + pin))
	#define _PORT_WRITE(pin, V) IO_REG8(pgm_read_byte(port_lu + pin)) = V
#elif defined(AVR_ATmega2560_FAMILY)
...
#if defined(AVR_ATmega328_FAMILY)
	#define PORT_READ(pin) _PORT_READ(pin)
	#define PORT_WRITE(pin, V) _PORT_WRITE(pin, V)
#endif

#define PIN_INPUT_PULLUP(pin) do{ _PIN_INPUT(pin); _PIN_WRITE(pin, 0x01); }while(0)
#define PIN_OUT_WRITE(pin, V) do{ _PIN_WRITE(pin, V); _PIN_OUTPUT(pin);  }while(0)
...
// 2560 pin mapping
//
#if defined(AVR_ATmega2560_FAMILY)
...
const uint16_t ddr_lu[87] PROGMEM {
	...
	0x2D, 0x2A, 0x2A, 0x2A, 0x101, 0x101, 0x3E
};
...
const uint16_t port_lu[87] PROGMEM {
	...
	0x2E, 0x2B, 0x2B, 0x2B, 0x102, 0x102, 0x3E
};
...
const uint16_t pin_lu[87] PROGMEM {
	...
	0x2C, 0x29, 0x29, 0x29, 0x100, 0x100, 0x3E
};
...
const uint8_t ddr_lu[22] PROGMEM {
	0x2A, 0x2A, 0x2A, 0x2A, 0x2A, 0x2A, 0x2A, 0x2A,
	0x24, 0x24, 0x24, 0x24, 0x24, 0x24, 0x27, 0x27,
	0x27, 0x27, 0x27, 0x27, 0x3E,
};

const uint8_t port_lu[22] PROGMEM {
	0x2B, 0x2B, 0x2B, 0x2B, 0x2B, 0x2B, 0x2B, 0x2B,
	0x25, 0x25, 0x25, 0x25, 0x25, 0x25, 0x28, 0x28,
	0x28, 0x28, 0x28, 0x28, 0x3E,
};

const uint8_t pin_lu[22] PROGMEM {
	0x29, 0x29, 0x29, 0x29, 0x29, 0x29, 0x29, 0x29,
	0x23, 0x23, 0x23, 0x23, 0x23, 0x23, 0x26, 0x26,
	0x26, 0x26, 0x26, 0x26, 0x3E,
};

const uint8_t bit_lu[22] PROGMEM {
	0x01, 0x02,	0x04, 0x08, 0x10, 0x20, 0x40, 0x80,
	0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x01, 0x02,
